...
attached at it.
The model involves a cubic lattice with all the
...
int sum=Link[x][y][z][0] - Link[Prev(x)][y][z][0]+Link[x][y][z][1]-Link[x][Prev(y)][z][1];
...
bool result=false;	//(new code)this checks whether we should keep the new configuration
			//based on flux conservation
if(local_flux == 1){
*my_Q=1;
*my_B=0;
result=true;
...
int new_local_flux= dFlux-Link[Prev(x)][y][z][0]-Link[x][Prev(y)][z][1]-Link[x][y][Prev(z)][2];
int new_Q, new_B;
bool consistency_check=false;		
consistency_check= place_vertex(new_local_flux,x,y,z, &new_Q, &new_B);	//(new line)check whether the new flux line
									//is consistence with the flux 
									//conservation
...
// More Observables
void measure_vertices(double *qnum, double *qbarnum, double *bnum, double *bbarnum){
*qnum=0.0;
*qbarnum=0.0;
*bnum=0.0;
*bbarnum=0.0;
...
ofstream 	energy("phasem_1.0new30.data");
ofstream	cv("CVm_1.0new30.data");
//ofstream	Qfile("m_0.0quark30.data");	//no need of these 
//ofstream	Bfile("m_0.0baryon30.data");	//to watch the behaviour of CV
// Quark Mass
m=1.0;
// Y-Vertex Energy
v=0.0;
...
for ( beta=1.5; beta <=3; beta= beta+0.05){	//To find the precise point(of beta) for which 
//beta=2.0;					//the peak arises.
qnum=0.0;
qbarnum=0.0;
bnum=0.0;
bbarnum=0.0;
...
//double qav, qav_err,qbarav, qbarav_err;	//Not necessary anymore, as they
//double bav, bav_err, bbarav, bbarav_err;	//irrelevant to watching CV behaviour
...
//Qfile<< beta << " " << qav<< " "<< qav_err<< " " << qbarav<< " " << qbarav_err<< endl;	//again same reason, not for CV
//Bfile<< beta << " " << bav<< " "<< bav_err<< " " << bbarav<< " " << bbarav_err<< endl;
}
return(0);
}
