...
int main(void) {
  ...
  for (beta = 1.5; beta <= 3;
       beta = beta + 0.05) { // To find the precise point(of beta) for which
    ...
    // double bav, bav_err, bbarav, bbarav_err;	//irrelevant to watching CV
    // behaviour multijack_average(quark, &qav, &qav_err);
    ...
    // qbarav_err<< endl;	//again same reason, not for CV Bfile<< beta <<
    // " " << bav<< " "<< bav_err<< " " << bbarav<< " " << bbarav_err<< endl;
  }
  return (0);
}
